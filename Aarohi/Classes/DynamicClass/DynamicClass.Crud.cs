            ...
            return SafeExecute("BULK_UPSERT_DT", extras =>
            {
                ...
                string BuildSqlType(ColumnInfo ci)
                {
                    ...
                    if (t is "varchar" or "nvarchar" or "char" or "nchar" or "varbinary" or "binary")
                    {
                        ...
                        if (t is "nvarchar" or "nchar") len = len / 2;
                        if (len <= 0) len = 1;
                        return $"{t}({len})";
                    }
...
FROM @Actions;";
        ...
        /// <summary>
        /// Saves the current <see cref="Values"/> to the database (inserting if new, updating changed columns if existing)
        /// and automatically records a detailed audit log of any modifications (old values vs. new values) in the AppLogs table.
        /// </summary>
        /// <param name="askBeforeOverwrite">If set to true, displays a visual confirmation dialog detailing the changes to the user before writing updates to the database.</param>
        /// <param name="Warningneeded">If set to true, displays a warning popup dialog to the user if the database indicates no rows were updated (e.g., if the data matches current values exactly).</param>
        /// <returns>Returns the primary key/identity value of the saved row, or <c>null</c> if the user cancels or the operation fails.</returns>
        public object? SaveWithAudit(bool askBeforeOverwrite = true, bool Warningneeded = true) => SafeExecute("SAVE_WITH_AUDIT", extras =>
        {
            var columns = GetColumns() ?? new List<ColumnInfo>();
            var physicalTable = ResolveTableName();
            var physicalKeyColumn = ResolveKeyColumnName(columns);
            ...
            if (string.IsNullOrWhiteSpace(physicalKeyColumn))
            {
                ...
                WriteLog(LogLevel.Info, $"Inserted new record in [{Schema}].[{physicalTable}] (No key configured)", LogSource, null, extras);
            ...
            if (!TryGetValueForColumn(physicalKeyColumn, columns, out var keyVal) || IsEmptyKey(keyVal))
            {
                ...
                // Log successful insertion
                WriteLog(LogLevel.Info, $"Inserted new record in [{Schema}].[{physicalTable}] with key '{id1}'", LogSource, null, extras);
            ...
            if (exists)
            {
                ...
                // SAFETY AUDIT: Capture details of changes in a fail-safe try-catch block
                try
                {
                    using (var cn2 = Open())
                    {
                        var existingRow = GetRowByKey(cn2, keyVal);
                        changes = existingRow != null ? BuildChangeSummary(existingRow) : "(Could not read existing row)";
                    }
                }
                catch (Exception auditEx)
                {
                    changes = $"(Audit failed: {auditEx.Message})";
                }
                ...
                AddCommonExtras(extras, ("mode", "update"), ("key", keyVal), ("affected", affected), ("changes", changes));

                // Log successful update with changes
                WriteLog(LogLevel.Info, $"Updated record in [{Schema}].[{physicalTable}] for key '{keyVal}'. Changes: {changes}", LogSource, null, extras);
            ...
            else
            {
                ...
                // Log successful insertion
                WriteLog(LogLevel.Info, $"Inserted new record in [{Schema}].[{physicalTable}] with key '{id2}'", LogSource, null, extras);
